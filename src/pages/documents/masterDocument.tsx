  ...
  return (
    <div className="p-2">
      <div className="flex flex-col gap-3 mb-3">
        <Accordion title={dataMaster[0]?.helpStatus === 'SERVICE_SPECIFICATION' ? t('Service_Specification') : t('Offer')}>
          {trueFalse(dataMaster[0]?.helpStatus) && dataMaster[0]?.text && (
            <div
          ...
          <div
            className={`mt-3 ${trueFalse(dataMaster[0]?.helpStatus) && 'flex justify-end items-center'} text-slate-700 dark:text-slate-300`}>
            ...
            {/*  />*/}
            {/*</div>*/}
            {trueFalse(dataMaster[0]?.helpStatus) && (
              <button
                onClick={() => {
                  setUpdateTextArea(dataMaster[0]);
                  setModalVal({ text: dataMaster[0]?.text || '', active: dataMaster[0]?.active });
                  openModal();
                }}
