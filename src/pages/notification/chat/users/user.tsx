      ...
      <ul>
        {chatData && chatData.length > 0 ?
          chatData.map((item: any) =>
            <li key={i++} onClick={() => {
              if (stomp && stomp.connected) setUserId(item.userId);
              else alert(t('Stomp_client_not_connected'));
            }}
              ...
              <img
                src={item.attachmentId ? getFileId + item.attachmentId : admin}
                alt="user img"
              ...
              <div className={`flex gap-1 flex-col w-full relative`}>
                <div className="flex gap-3 justify-between w-full">
                  ...
                  <div className="flex">
                    {item.chatDto.createdAt ? moment(item.chatDto.createdAt).format('DD.MM.YYYY') : t('was_recently_oline')}
                  </div>
                </div>
                ...
                {item.newMessageCount > 0 && <div
                  className="absolute right-3 -top-4 bg-green-600 text-white text-xs w-4 h-4 flex justify-center items-center rounded-full">{item.newMessageCount}</div>}
              </div>
            </li>
          ) :
