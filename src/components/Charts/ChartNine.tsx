...
const ChartNine: React.FC = () => {
  ...
  useEffect(() => {
    if (chart.length > 0) {
      const maxIncome = Math.max(...chart.map((item) => item?.incomeTotal));

      setState({
        series: [
          {
            name: 'Income',
            data: chart.map((item) => item?.incomeTotal),
          },
        ],
      });
      ...
      setOptions((prevOptions) => ({
        ...
        yaxis: {
          ...prevOptions.yaxis,
          max: maxIncome + maxIncome * 0.1, // Apidan kelgan qiymatlarni kattasiga 10% qushadi max uchun
        },
      }));
    }
  }, [chart]);
